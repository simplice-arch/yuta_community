...
function makeFruit(name) {
  return { name, beli: BELI_PRICES[name] || 0, type: TYPE_MAP[name] || "Natural" };
}
...
async function fetchFromFandom() {
  ...
  for (const line of lines) {
    ...
    if (!m) continue;
    const name = m[1].trim();
    if (name && name.length > 1) normal.push(makeFruit(name));
  }

  if (normal.length === 0) {
    const csMatch = wikitext.match(/Current Stock[:\s]+([^\n\r]+)/i);
    if (csMatch) {
      csMatch[1].split(/[,\s]+/).map(f => f.trim())
        .filter(f => f && BELI_PRICES[f] !== undefined)
        .forEach(f => normal.push(makeFruit(f)));
    }
  }
...
export default async function handler(req) {
  try {
    ...
    console.log("[/api/stock] OK:", data.normal.map(f=>f.name).join(", "));
